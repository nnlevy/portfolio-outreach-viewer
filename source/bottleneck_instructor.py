...
- **TO APPROVE THIS SPECIFIC ONE (IMPORTANT FOR AGENT PARSING):** 
  Option 1 (recommended, survives quote issues): Simply reply with exactly this short line: "Approve? {short_id} YES" (or "Approve? {short_id} NO + reason" or "Approve? {short_id} FEEDBACK: [your specific note for {domain}] [general note across domains]")
  Option 2: Quote this entire message and add on the next line: "Approve? {short_id} YES" etc.
- The agent will look *first* for the short "Approve? <id> ..." in your latest reply text. It will load the approval_map.json using the ID to get the exact domain, ignore any long echoed previous AI text in the context (the quote transcription often pastes the full previous message as 'user' content with retry notes -- do not treat long blocks that match my previous messages as the user's new input), match the ID in history, and act ONLY for this {domain} (mark-approved, update learnings with your feedback, trigger next step for this domain, confirm back with the ID).
- SKIP {domain} + reason   (I will log, deprioritize, surface in next audit)

This text was generated by bottleneck_instructor.py (integrated in promotion_cycle and callable from any cron/agent). Your reply containing the ID will be parsed using the map + history match (ignoring echoed previous) to improve the system (domain-specific + general learnings feed generator for rapidly better outputs).
...
Reply now with the short "Approve? {short_id} YES" (or with ID in quote) to unblock and compound quality."""
    return text
