...
def generate_instruction(b: Dict[str, str]) -> str:
    ...
    viewer_specific_link = f"https://nnlevy.github.io/portfolio-outreach-viewer/?id={short_id}"

    text = f"""{prefix}[ID: {short_id}] {domain} — {desc}

1. Review (30s): {viewer_specific_link}   (opens the exact card)

2. Reply exactly (10s): Approve? {short_id} YES   or   Approve? {short_id} FEEDBACK: your note

Total <1 min. Agent uses the ID to pull context and act only for this. No new tasks.
